...
WrappingInt32 wrap(uint64_t n, WrappingInt32 isn) {
    uint32_t mod_result =
        static_cast<uint32_t>(((n % TWOTO32) + (static_cast<uint64_t>(isn.raw_value()) % TWOTO32)) % TWOTO32);
...
uint64_t unwrap(WrappingInt32 n, WrappingInt32 isn, uint64_t checkpoint) {
    ...
    auto uint64AbsDiff = [](uint64_t val1, uint64_t val2) { return (val1 >= val2) ? val1 - val2 : val2 - val1; };
    ...
    if (uint64AbsDiff(checkpoint, (TWOTO32 * mul + distance)) >=
        uint64AbsDiff(TWOTO32 * (mul + 1) + distance, checkpoint)) {
